...
export default function HeroSlider() {
  ...
  useEffect(() => {
    ...
    }, 3500); // 👈 change to 3000 or 4000 if needed
  ...
  return (
    <div className="relative w-full md:w-[80%] lg:w-[80%] h-[70vh] overflow-hidden rounded-2xl mx-auto">
      {/* Slides */}
      {slides.map((slide, index) => (
        ...
        >
          {/* Background Image */}
          <div
            className="w-full h-full bg-cover bg-center min-h-100"
          ...
          <div className="absolute inset-0 flex items-center justify-center text-center px-4">
            <div className="text-white max-w-2xl">
              ...
              <p className="text-sm text-gray-200">{slide.meta}</p>
            </div>
          </div>
        </div>
      ))}
      ...
      <button
        onClick={prevSlide}
        className="absolute bottom-6 left-1/2 -translate-x-12 bg-white/80 hover:bg-white p-2 rounded-full transition"
      >
      ...
      <button
        onClick={nextSlide}
        className="absolute bottom-6 left-1/2 translate-x-2 bg-white/80 hover:bg-white p-2 rounded-full transition"
      >
